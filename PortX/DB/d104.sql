...
invoice_fetch:LOOP
	...
	DECLARE invoice_info CURSOR for select invoice_details.container_id, invoice_details.product_key
                  ...
                  where invoice.id = invoiceid order by invoice_details.id desc;
        OPEN invoice_info;
    details_fetch:LOOP
    	FETCH invoice_info into containerid, product_id;
        IF containerid is NULL THEN
  			LEAVE details_fetch;
 	 	END IF;
    	SELECT id into activity FROM `container_log`
                      WHERE container_log.container_id=containerid and invoiced = 1 and activity_id = product_id
                      ORDER BY id DESC LIMIT 1;
    	UPDATE container_log set container_log.invoiced = 0 where id = activity;
    	DELETE from invoice_container WHERE invoice_container.invoice_id = invoiceid and invoice_container.container_id =  containerid;
...
invoice_fetch:LOOP
	...
	DECLARE invoice_info CURSOR for select supplementary_invoice_details.container_id,supplementary_invoice_details.product_key
                  ...
                  where supplementary_invoice.id = invoiceid order by supplementary_invoice_details.id desc;
        OPEN invoice_info;
    details_fetch:LOOP
    	FETCH invoice_info into containerid, product_id;
        IF containerid is NULL THEN
    	...
    	SELECT id into activity FROM `container_log`
                      ...
                      ORDER BY id DESC LIMIT 1;
    	UPDATE container_log set container_log.invoiced = 0 where id = activity;
    	DELETE from supplementary_invoice_container WHERE supplementary_invoice_container.invoice_id = invoiceid and supplementary_invoice_container.container_id =  containerid;
	SET containerid = NULL;
	SET product_id = NULL;
	SET activity = NULL;
    END LOOP;
...
DELIMITER $$
CREATE EVENT `INVOICE_UPDATE` ON SCHEDULE EVERY 1 DAY STARTS '2020-07-01 23:58:00.000000' ON COMPLETION PRESERVE ENABLE DO BEGIN CALL INVOICE_EXPIRE(); CALL SUPP_INVOICE_EXPIRE();
END $$
DELIMITER ;