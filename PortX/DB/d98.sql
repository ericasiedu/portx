...
BLOCK0:BEGIN
    ...
    DECLARE expired_supp CURSOR for SELECT supplementary_invoice.id from supplementary_invoice where supplementary_invoice.due_date <= CURRENT_DATE && status = 'UNPAID';
...
CLOSE expired;
	...
	SET info_fetch_state =true;
	OPEN expired_supp;
supp_invoice_fetch:LOOP
	FETCH expired_supp into invoiceid;
    IF inv_fetch_state = FALSE THEN
  		LEAVE supp_invoice_fetch;
 	END IF;
	BLOCK1: Begin
...
DELIMITER $$
CREATE EVENT `INVOICE_UPDATE` ON SCHEDULE EVERY 1 DAY STARTS '2020-07-01 23:58:00.000000' ON COMPLETION PRESERVE ENABLE DO BEGIN CALL INVOICE_EXPIRE();
END $$
DELIMITER ;