...
class ExportInvoice{
    ...
    public function show_export($invoice_number){
        ...
        $query->query("SELECT invoice.status, date AS check_date, cost AS subtotal, waiver_pct, waiver_amount,invoice.id AS invoice_id, currency.code AS currency, invoice.book_number, customer.name as cust,
        ...
        $query->query("SELECT cost, description, total_cost, qty FROM invoice_details WHERE invoice_id =?");
        $query->bind = array('i',&$invoice_id);
        ...
        $pdf->AliasNbPages();
        $pdf->AddPage();
        $pdf->SetTitle( $trade_type == 4 ? 'Export Invoice': 'Empty Invoice');
        // $this->setTitle($pdf);
