...
class ImportInvoice{
    ...
    public $trade_title = "Import Invoice";
    ...
    public function show_import($invoice_number){
        ...
        $query->query("SELECT invoice.status,invoice.boe_number,invoice.release_instructions, date AS check_date, cost AS subtotal, waiver_pct, waiver_amount, invoice.id AS invoice_id, currency.code AS currency, invoice.bl_number, customer.name as cust, 
            invoice.bill_date, invoice.due_date, invoice.cost, invoice.do_number, invoice.tax, invoice.tax_type, user_id FROM invoice 
            INNER JOIN currency ON invoice.currency = currency.id
            INNER JOIN customer on invoice.customer_id  = customer.id WHERE invoice.number =?");
        ...
        $query->query("SELECT COUNT(distinct(container_id)) AS qty FROM invoice_details WHERE invoice_id =?");
        ...
        $query->query("SELECT qty, cost, description, total_cost FROM invoice_details WHERE invoice_id =?");
        $query->bind = array('i',&$invoice_id);
        $details_query = $query->run();


        $waive_query = new MyQuery();
        $waive_query->query("select sum(cost) as waive_total from invoice_details where invoice_id='$invoice_id'");
        $waive_query->run();
        ...
        $pdf->invoice_title = "CASH DELIVERY INVOICE";
        ...
        $pdf->goods_description = html_entity_decode($pdf->goods_description);
        ...
        $pdf->secondBody();
        $pdf->SetFont('Arial','',8.8);
        $query->query("SELECT distinct(container.number), container.content_of_goods, container_isotype_code.code, container_isotype_code.length FROM invoice_details INNER JOIN invoice 
            ON invoice.id = invoice_details.invoice_id INNER JOIN container ON container.id = invoice_details.container_id 
            INNER JOIN container_isotype_code ON container_isotype_code.id = container.iso_type_code WHERE invoice.id =?");
        $query->bind = array('i',&$invoice_id);
        $query->run();
