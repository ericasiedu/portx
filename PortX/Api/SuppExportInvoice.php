...
class SuppExportInvoice
{
    ...
    public function show_export($invoice_number)
    {
        ...
        $query->query("SELECT supplementary_invoice.status, invoice.number AS main_invoice, supplementary_invoice.date AS check_date, supplementary_invoice.cost AS subtotal,supplementary_invoice.waiver_pct, supplementary_invoice.waiver_amount,supplementary_invoice.id AS invoice_id, currency.code 
              AS currency, invoice.book_number,customer.name AS customer, invoice.bill_date, supplementary_invoice.due_date, supplementary_invoice.cost, invoice.do_number, supplementary_invoice.tax, invoice.tax_type, supplementary_invoice.user_id
        ...
        $docInfo = $this->getTradeInfo('',$book_number);
        ...
        $query->query("SELECT cost, description, total_cost, qty FROM supplementary_invoice_details WHERE invoice_id =?");
        $query->bind = array('i',&$invoice_id);
        $query->run();
        ...
        $pdf->AliasNbPages();
        $pdf->AddPage();
        // $pdf->SetTitle('Supplementary Export Invoice');
        $this->setTitle($pdf);
        ...
        $query->query("SELECT container.number, container.content_of_goods, container_isotype_code.code, container_isotype_code.length 
              ...
              INNER JOIN container_isotype_code ON container_isotype_code.id = container.iso_type_code WHERE supplementary_invoice_details.invoice_id =?");
        $query->bind = array('i',&$invoice_id);
        $query->run();
        $bullet_number = 1;
    ...
    function getTradeInfo($container_id,$book_number) {
        $info = new DocumentInfo();
        return $info->getExportInvoiceInfo($book_number);
    }
