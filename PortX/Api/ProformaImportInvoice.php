...
class ProformaImportInvoice{
    private $request,$response;
    public $trade_title = 'Proforma Import Invoice';
    ...
    public function show_import($invoice_number){
        ...
        $query->query("SELECT proforma_invoice.boe_number,proforma_invoice.release_instructions, date AS check_date, cost AS subtotal, waiver_pct, waiver_amount, proforma_invoice.id AS invoice_id, currency.code AS currency, proforma_invoice.bl_number, customer.name as cust, 
        ...
        $query->query("SELECT qty, cost, description, total_cost FROM proforma_invoice_details WHERE invoice_id =?");
        $query->bind = array('i',&$invoice_id);
