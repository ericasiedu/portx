...
class SuppImportInvoice{
    private $request,$response;
    public $trade_title = "Supplementary Import Invoice";
    ...
    public function show_import($invoice_number){
        ...
        $query->query("SELECT supplementary_invoice.status,invoice.boe_number, invoice.number AS main_invoice,invoice.release_instructions, supplementary_invoice.date 
              AS check_date, supplementary_invoice.cost AS subtotal,supplementary_invoice.waiver_pct, supplementary_invoice.waiver_amount, supplementary_invoice.id AS invoice_id, currency.code 
              AS currency, invoice.bl_number, invoice.bill_date,customer.name AS customer, supplementary_invoice.due_date, supplementary_invoice.cost, invoice.do_number, supplementary_invoice.tax, invoice.tax_type, supplementary_invoice.user_id
              FROM supplementary_invoice INNER JOIN invoice ON invoice.id = supplementary_invoice.invoice_id 
              INNER JOIN currency ON currency.id = invoice.currency INNER JOIN customer ON customer.id = invoice.customer_id WHERE supplementary_invoice.number =?");
        ...
        $query->query("SELECT COUNT(distinct(container_id)) AS qty FROM supplementary_invoice_details WHERE invoice_id =?");
        ...
        $query->query("SELECT qty, cost, description, total_cost FROM supplementary_invoice_details WHERE invoice_id =?");
        $query->bind = array('i',&$invoice_id);
        $query->run();
        ...
        if ($taxType == 1 || $taxType == 2 || $taxType == 4){
            $tax_tbl = 'supplementary_invoice_details_tax';
        ...
        $query->query("SELECT distinct(container.number), container.content_of_goods, container_isotype_code.code, container_isotype_code.length 
              FROM supplementary_invoice_details INNER JOIN container ON container.id = supplementary_invoice_details.container_id 
              INNER JOIN container_isotype_code ON container_isotype_code.id = container.iso_type_code WHERE supplementary_invoice_details.invoice_id = '$invoice_id'");
        $query->run();
        $bullet_number = 1;
